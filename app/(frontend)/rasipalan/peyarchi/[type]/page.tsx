...
interface PeyarchiContentProps {
  type: string;
}

function PeyarchiContent({ type }: PeyarchiContentProps) {
  ...
  const targetType = `peyarchi_${type}`;

  const typeTitles: Record<string, string> = {
    guru: t('குரு பெயர்ச்சி'),
    sani: t('சனி பெயர்ச்சி')
  };
  ...
  useEffect(() => {
    async function fetchPredictions() {
      ...
      try {
        ...
        console.error('Error fetching peyarchi horoscopes:', error);
      } finally {
        setLoading(false);
      }
    }
  ...
  const handleRasiSelect = (key: string) => {
    router.push(`/rasipalan/peyarchi/${type}?rasi=${key}`);
  };
...
export default function PeyarchiPage({ params }: Props) {
