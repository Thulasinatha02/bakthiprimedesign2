...
interface RasiContentProps {
  type: string;
}

function RasiPalanContent({ type }: RasiContentProps) {
  ...
  const typeTitles: Record<string, string> = {
    daily: t('இன்றைய 12 ராசிபலன்கள் (Daily Horoscope)'),
    weekly: t('வாராந்திர ராசிபலன்'),
    monthly: t('மாதாந்திர ராசிபலன்')
  };
  ...
  useEffect(() => {
    async function fetchPredictions() {
      setLoading(true);
      try {
        const res = await fetch(`/api/rasipalan?type=${type}`);
  ...
  useEffect(() => {
    async function fetchComments() {
      ...
      try {
        const res = await fetch(`/api/comments?targetKey=${selectedRasiKey}&targetType=${type}`);
        const json = await res.json();
        if (json.success) {
  ...
  const handleRasiSelect = (key: string) => {
    router.push(`/rasipalan/${type}?rasi=${key}`);
  };
          ...
          {loading ? (
            ...
            >
              ...
              <div className="flex items-center gap-2 text-xs font-bold text-amber-800 bg-amber-50 w-fit px-3 py-1.5 rounded-full">
                ...
                <span>காலம்: {activePrediction.date}</span>
              </div>
...
export default function RasiPalanTypePage({ params }: Props) {
