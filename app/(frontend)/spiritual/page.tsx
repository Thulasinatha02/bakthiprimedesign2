...
export default function SpiritualPage() {
  ...
  useEffect(() => {
    async function fetchArticles() {
      try {
        const res = await fetch('/api/news?category=Spiritual');
        const json = await res.json();
        if (json.success) {
          setArticles(json.data);
        }
      } catch (error) {
        console.error('Error fetching spiritual articles:', error);
      } finally {
        setLoading(false);
      }
    }
    ...
    <div className="max-w-7xl mx-auto px-4 py-12 sm:px-6 lg:px-8">
      ...
      <div className="border-b border-amber-900/10 pb-5 mb-8 flex flex-col sm:flex-row sm:items-end sm:justify-between gap-2">
        <div>
          ...
          <h1 className="text-3xl font-extrabold text-amber-950 font-sans tracking-wide">
            {t('ஆன்மிக செய்திகள் & கட்டுரைகள் (Spiritual News)')}
          </h1>
        </div>
        <p className="text-stone-500 text-sm">{t('வாழ்வை நல்வழிப்படுத்தும் தெய்வீக கட்டுரைகள்')}</p>
      </div>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-xl shadow-md p-12 text-center text-stone-500 border border-stone-100 max-w-md mx-auto">
          {t('தற்சமயம் ஆன்மிக கட்டுரைகள் எதுவும் பதிவிடப்படவில்லை.')}
        </div>
      ) : (
