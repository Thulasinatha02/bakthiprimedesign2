...
export default function AdminImportantNewsPage() {
  ...
  const fetchNews = async () => {
    setLoading(true);
    try {
      const res = await fetch('/api/news?isImportant=true');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const body = {
      ...
      image,
      youtubeUrl,
      isImportant: true, // Always true — this is the Important News page
      isLatest,
  ...
  const handleDelete = async (id: string) => {
    if (!confirm('இந்த முக்கிய செய்தியை நீக்க வேண்டுமா?')) return;
    ...
    <div className="space-y-8">
      ...
      <div className="flex justify-between items-center border-b border-amber-900/30 pb-6">
        <div>
          <h1 className="text-2xl sm:text-3xl font-extrabold text-amber-200 tracking-wide font-sans gold-glow-text">
            முக்கிய செய்திகள் மேலாண்மை (Important News)
          </h1>
          <p className="text-stone-400 text-xs mt-1 font-semibold">
            YouTube லிங்கை மட்டும் ஒட்டவும் — Thumbnail தானாக வரும். isImportant தானாகவே அமைக்கப்படும்.
          </p>
        </div>
        ...
        {!showForm && (
          ...
          >
            ...
            <span>புதிய முக்கிய செய்தி</span>
          </button>
        )}
      </div>
        ...
        >
          <div className="flex justify-between items-center border-b border-amber-900/20 pb-3 mb-5">
            <h2 className="text-lg font-bold text-amber-200">
              {editId ? 'முக்கிய செய்தியைத் தொகுக்கவும் (Edit)' : 'புதிய முக்கிய செய்தி (Add Important News)'}
            </h2>
          ...
          {/* Auto isImportant Badge */}
          <div className="mb-4 px-3 py-2 rounded-lg bg-red-500/10 border border-red-500/20 flex items-center gap-2">
            <Sparkles className="w-4 h-4 text-red-400" />
            <span className="text-xs font-bold text-red-300">isImportant = true — இந்தப் பக்கத்தில் சேர்க்கும் அனைத்து செய்திகளும் &quot;முக்கிய செய்தி&quot; ஆக அமைக்கப்படும்.</span>
          </div>
            ...
            <div className="bg-black/20 p-4 rounded-xl border border-yellow-500/5">
              <div className="flex items-center gap-2 cursor-pointer">
                ...
                <label htmlFor="isLatest" className="uppercase tracking-wider select-none cursor-pointer">
                  அண்மை செய்தியாகவும் அமை (Also set as Latest News)
                </label>
              </div>
            </div>
      ...
      {loading ? (
        <div className="text-center py-12 text-stone-400 font-bold animate-pulse text-sm">
          முக்கிய செய்திகள் ஏற்றப்படுகின்றன...
        </div>
      ) : news.length === 0 ? (
        <div className="glass-card rounded-2xl p-12 text-center text-stone-500 text-sm max-w-md">
          முக்கிய செய்திகள் எதுவும் இல்லை. புதிய செய்தி சேர்க்கவும்.
        </div>
      ) : (
