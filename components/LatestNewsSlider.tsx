...
export default function LatestNewsSlider() {
  ...
  useEffect(() => {
    async function fetchNews() {
      setLoading(true);
      try {
        let url = '/api/news?isLatest=true&limit=6';
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-6 border-b border-amber-900/10 pb-4">
          <div>
            <span className="text-xs font-bold text-orange-600 uppercase tracking-widest block mb-1">
              {language === 'ta' ? 'சமீபத்திய புதுப்பிப்புகள்' : 'Recent Updates'}
            </span>
          ...
          <div className="flex flex-wrap gap-2 items-center mt-4 md:mt-0">
            {categories.map((cat) => (
              <button
                ...
                className={`px-3 py-1.5 rounded-full text-xs font-bold transition duration-200 cursor-pointer shadow-sm ${selectedCategory === cat.value
          ...
          <AnimatePresence mode="wait">
            {loading ? (
              ...
              <div className="bg-white rounded-2xl p-12 border border-stone-100 text-center text-stone-500 text-sm max-w-md mx-auto flex flex-col items-center justify-center space-y-2">
                ...
                <p className="font-bold">{language === 'ta' ? 'இந்தப் பிரிவில் தற்சமயம் செய்திகள் ஏதுமில்லை.' : 'No latest news in this category at the moment.'}</p>
              </div>
            ) : (
              <motion.div
                initial={{ opacity: 0 }}
